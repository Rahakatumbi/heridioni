<template>
    <v-app>
        <subheader title="Rapport d'Approvisionnement">
                <v-spacer></v-spacer>
                    ...
                    <v-col md="9" cols="12">
                        <v-row>
                            <v-col cols="12" v-if="rapport.type" md="4">
                                ...
                                :items="branches"
                                item-value="id" label="Au" outlined placeholder="Jusqu'au">      
                ...
                <template v-slot:body="{items}">
                    <tbody v-if="items.length">
                        <tr v-for="(item,index) in items" :key="index">
                            ...
                            <td>{{item.branche +" Code: 0BH" +item.branche_id}}</td>
                            <td>{{item.supplier}}</td>
...
export default {
    ...
    data(){
        return{
            ...
            branches:[],
            loading:false,
            headers:[{text:'N#'},{text:'Item'},{text:'Qte'},{text:'Qualite'},{text:'P.U'},{text:'Total'},{text:'Depot Code'},
            {text:'Cultivateur'},{text:'Date'},{text:'Fait Far'}],
    ...
    methods:{
        ...
        async generate(){
            this.loading = true
            this.data = await (await rapportServices.approvisionnement(this.rapport)).data
            this.loading =false
        }
    },
